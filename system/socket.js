  ...
  const sock = Object.defineProperties(conn, {
    ...
    getName: {
      value(jid = "", withoutContact = false) {
        ...
        if (jid.endsWith("g.us")) return new Promise(async resolve => {
          v = store.chats[jid] || {};
          if (!(v.name || v.subject)) v = await store.groupMetadata[jid] || {};
          resolve(v.name || v.subject || PhoneNumber("+" + jid.replace("@s.whatsapp.net", "")).getNumber("international"));
        });
        else v = jid === "0@s.whatsapp.net" ? {
          jid: jid,
          vname: "WhatsApp"
        } : areJidsSameUser(jid, conn.user.id) ? conn.user : store.chats[jid] || {};
    ...
    getFile: {
      async value(PATH, returnAsFilename) {
        let res, filename;
        const data = Buffer.isBuffer(PATH) ? PATH : /^data:.*?\/.*?base64,/i.test(PATH) ? Buffer.from(PATH.split`,` [1], "base64") : /^https?:\/\//.test(PATH) ? await (res = await Func.fetchBuffer(PATH)) : fs.existsSync(PATH) ? (filename = PATH, fs.readFileSync(PATH)) : typeof PATH === "string" ? PATH : Buffer.alloc(0);
    ...
    sendFile: {
      async value(jid, path, filename = "", caption = "", quoted, ptt = false, options = {}) {
        ...
        else if (/audio/.test(type.mime)) convert = await (ptt ? toPTT : toAudio)(file, type.ext),
    ...
    downloadMediaMessage: {
      async value(message, filename) {
        ...
        if (filename) {
          ...
          let filePath = path.join(process.cwd() + "/tmp", `${filename}.${mime.ext}`);
        ...
        if (json.url) {
          ...
          let mediaMessage = await prepareWAMessageMedia({
            ...mime === "image" ? {
              ...
              document: file.data,
              mimetype: file.mime,
              fileName: json.filename || "Yoshida." + extension(file.mime)
            }
          }, {
            upload: conn.waUploadToServer
          });
          msg = generateWAMessageFromContent(jid, {
            viewOnceMessage: {
              message: {
                messageContextInfo: {
                  deviceListMetadata: {},
...
async function serialize(conn, msg, store) {
  ...
  if (msg.key) {
    ...
    m.isBot = m.id.startsWith("BAE") || m.id.startsWith("B1E") || m.id.startsWith("FELZ") || m.id.startsWith("3EB") || m.id.startsWith("WA") || m.id.startsWith("Fizz");
  ...
  if (m.message) {
    ...
    m.timestamps = typeof msg.messageTimestamp === "number" ? msg.messageTimestamp * 1e3 : m.msg.timestampMs * 1e3;
    ...
    if (m.msg?.contextInfo?.quotedMessage) {
      ...
      if (m.quoted.message) {
        ...
        m.quoted.isBot = m.quoted.id.startsWith("BAE5") || m.quoted.id.startsWith("3EB") || m.quoted.id.startsWith("FELZ") || m.quoted.id.startsWith("B1E") || m.quoted.id.startsWith("Fizz");
        ...
        m.getQuotedObj = m.getQuotedMessage = async () => {
          ...
          let q = proto.WebMessageInfo.fromObject(await store.loadMessage(m.chat, m.quoted.id) || vM);

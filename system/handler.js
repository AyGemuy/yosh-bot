...
module.exports = async (conn, m, store) => {
  try {
    ...
    if (m.isBot) return;
    if (setting.debug_mode && !m.fromMe && isOwner) await m.reply(Func.jsonFormat(m));
    if (m.isGroup && !groupSet.stay && new Date() * 1 >= groupSet.expired && groupSet.expired != 0) {
      return conn.sendMessage(m.chat, {
        text: Func.texted("bold", "Bot time has expired and will leave from this group, thank you."),
        mentions: m.metadata.participants.map(v => v.id)
      }, {
        ephemeralExpiration: m.expiration
      }).then(async () => {
        groupSet.expired = 0;
        await Func.delay(3e3).then(() => conn.groupLeave(m.chat));
      });
    }
    if (users && new Date() * 1 >= users.expired && users.expired != 0) {
      return conn.sendMessage(m.sender, {
        text: Func.texted("bold", "Your premium package has expired, thank you for buying and using our service.")
      }, {
        ephemeralExpiration: m.expiration
      }).then(async () => {
        users.premium = false;
        users.expired = 0;
        users.limit = config.LIMIT;
      });
    }
    cron.schedule("00 00 * * *", () => {
      setting.lastreset = Date.now();
      Object.values(db.users).forEach(v => {
        if (v.limit < config.LIMIT && !v.premium) {
          v.limit = config.LIMIT;
        }
      });
    ...
    for (let name in plugins) {
      ...
      if (usedPrefix = (match[0] || "")[0]) {
        ...
        if (setting.private_mode && !isOwner && !m.fromMe && m.isGroup) continue;
        if (!m.isGroup && !["profile.js", "sewa.js", "_creator.js", "_rules.js", "_respon.js", "regist.js"].includes(name.split("/").pop()) && chats && !isPrems && !users.banned && new Date() * 1 - chats.lastchat < config.TIMEOUT) continue;
        if (!m.isGroup && !["profile.js", "sewa.js", "_creator.js", "_rules.js", "_respon.js", "regist.js"].includes(name.split("/").pop()) && chats && !isPrems && !users.banned && setting.group_mode && !Object.values((await conn.groupMetadata(config.ID_GC)).participants).find(users => users.id == m.sender)) {
          m.reply(mess["gconly"].replace("+link", setting.link)).then(async () => chats.lastchat = new Date() * 1);
          continue;
        }
        ...
        if (plugin.owner && !isOwner) {
          ...
          m.reply(`Limit penggunaan anda telah mencapai batas, silahkan tunggu pukul *00:00* untuk mereset limit anda, atau upgrade ke premium untuk mendapatkan *unlimited* limit.`).then(() => users.premium = false);
          continue;
          ...
          if (users.limit >= limit) {
            ...
            m.reply(Func.texted("bold", `Your limit is not enough to use this feature.`));
            continue;
          }
        } else if (plugin.level > users.level) {
          m.reply(`level *${plugin.level}* is required to use this command. Your level *${users.level}*`);
          continue;
        }
        let extra = {
          API: API,
...
${teks}`.trim()
    ...
    if (m.message && !m.fromMe) {
      ...
      console.log(`   - Date: ${new Date().toLocaleString("id-ID")} WIB \n` + `   - Message: ${m.body || m.type} \n` + `   - Sender Number: ${await conn.getName(m.sender)} \n` + `   - Sender Name: ${m.name} \n` + `   - Sender ID: ${m.id}`);
      if (m.isGroup) {
        console.log(`   - Group: ${await conn.getName(m.chat)} \n` + `   - GroupID: ${m.chat}`);
      }
    }
  }
};

  ...
  run: async (m, {
    ...
    try {
      if (m.command === "tt" || m.command === "tiktok") {
        const getRes = await Func.fetchJson(API("yosh", "/downloader/tiktok", {
          url: quoted.text
        }));
        const {
          data
      ...
      } else if (m.command === "tiktokmp3" || m.command === "tiktokaudio" || m.command === "ttmp3" || m.command === "ttaudio") {
        const getAudio = await Func.fetchJson(API("yosh", "/downloader/tiktok", {
          url: quoted.text
        }));
        const {
          data

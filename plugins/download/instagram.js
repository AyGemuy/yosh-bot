module.exports = {
  ...
  run: async (m, {
    ...
    m.react("⏱️");
    try {
      const data = await Func.fetchJson(API("yosh", "/downloader/instagram", {
        url: quoted.text
      }));
      ...
      const cap = Object.entries(metadata || {}).map(([a, b]) => `- ${a} : ${b}`).join("\n");
      for (let i = 0; i < data.result.url.length; i++) {
        try {
          ...
          await conn.sendFile(m.chat, buffer, "", i === 0 ? cap : "", m, {}, {

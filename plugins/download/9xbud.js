...
class XBuddyDownloader {
  ...
  async uploadToQuax(buffer, filename = "video.mp4") {
    ...
    try {
      ...
      console.log("Sukses mengunggah ke qu.ax:", result);
      return result;
    } catch (error) {
  ...
  async downloadAndUpload(youtubeUrl, resolution = "360p") {
    try {
...
module.exports = {
  ...
  run: async (m, {
    ...
    if (!quoted || !quoted.text) {
      m.react("ℹ️");
      return m.reply(Func.texted("bold", `Usage: Reply to a text message with .xbud to use xbud AI.`));
    }
    const [url, quality] = quoted.text.split(" ");
    m.react("⏱️");
    try {
      const downloader = new XBuddyDownloader();
      const result = await downloader.downloadAndUpload(url, quality);
      if (result) {
        m.react("✅");
        await conn.sendFile(m.chat, result, "", "", m);
      } else {
        m.react("❌");
        return m.reply(Func.texted("bold", "No result received from xbud AI."));
      }
    } catch (error) {
      console.error(error);
      m.react("❌");

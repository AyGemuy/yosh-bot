module.exports = {
  ...
  run: async (m, {
    ...
    m.react("⏱️");
    try {
      const data = await Func.fetchJson(API("yosh", "/downloader/facebook", {
        url: quoted.text
      }));
      if (!data.status) {
      ...
      if (data.result.comments && Array.isArray(data.result.comments) && data.result.comments.length > 0) {
        caption += "\n— *Top Comments:*\n";
      ...
      if (data.result.type === "image" && Array.isArray(data.result.image) && data.result.image.length > 0) {
      ...
      if (data.result.type === "video" && (data.result.hd || data.result.sd)) {

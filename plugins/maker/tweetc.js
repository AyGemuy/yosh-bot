module.exports = {
  ...
  run: async (m, {
    ...
    m.react("⏱️");
    try {
      const avatar = await conn.profilePictureUrl(m.sender, "image").catch(_ => "https://files.catbox.moe/ka3kec.jpg");
      const username = m.sender.split("@")[0];
      const replies = Math.floor(Math.random() * 1e3);
      const retweets = Math.floor(Math.random() * 1e3);

...
module.exports = {
  help: ["audio8d", "distortion", "flanger", "chorus", "reverb", "bass", "blown", "deep", "earrape", "speed", "nightcore", "reverse", "robot", "slow", "smooth", "chipmunk", "vibra", "pitch", "highpass", "lowpass", "underwater"],
  tags: ["converter"],
  command: ["audio8d", "distortion", "flanger", "chorus", "reverb", "bass", "blown", "deep", "earrape", "speed", "nightcore", "reverse", "robot", "slow", "smooth", "chipmunk", "vibra", "pitch", "highpass", "lowpass", "underwater"],
  run: async (m, {
    Func,
    quoted,
    conn
    ...
    try {
      ...
      if (!/audio/.test(mime)) return m.reply(Func.texted("bold", "Reply audio."));
      ...
      if (/speed/.test(m.command)) set = '-filter:a "atempo=1.63,asetrate=44100"';
      ...
      if (/chipmunk|squirrel|chipmunk/.test(m.command)) set = '-filter:a "atempo=0.5,asetrate=65100"';
      if (/vibra/.test(m.command)) set = '-filter_complex "vibrato=f=15"';
      if (/reverb/.test(m.command)) set = '-filter:a "aecho=0.8:0.9:1000:0.3"';
      if (/chorus/.test(m.command)) set = '-filter:a "chorus=0.7:0.9:55:0.4:0.25:2"';
      if (/flanger/.test(m.command)) set = '-filter:a "flanger=delay=20:depth=0.2"';
      if (/distortion/.test(m.command)) set = "-filter:a \"aecho=0.8:0.9:1000:0.3,firequalizer=gain_entry='entry(0,15);entry(250,0);entry(4000,15)'\"";
      if (/pitch/.test(m.command)) set = '-filter:a "asetrate=44100*1.25,atempo=1.25"';
      if (/highpass/.test(m.command)) set = '-filter:a "highpass=f=500"';
      if (/lowpass/.test(m.command)) set = '-filter:a "lowpass=f=500"';
      if (/underwater/.test(m.command)) set = '-af "asetrate=44100*0.5,atempo=2,lowpass=f=300"';

...
module.exports = {
  ...
  run: async (m, {
    ...
    try {
      if (!m.text) return m.reply(Func.example(m.prefix, m.command, "hai | bro"));
      let [top, bottom] = m.text.split`|`;
      let mime = (quoted.msg || quoted).mimetype || "";
      if (!mime) return m.reply(Func.texted("bold", `Reply photo.`));
      if (!/webp|image\/(jpe?g|png)/.test(mime)) return m.reply(Func.texted("bold", `Media is not supported, can only be pictures and stickers.`));

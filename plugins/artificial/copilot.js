...
module.exports = {
  ...
  run: async (m, {
    ...
    if (!quoted || !quoted.text) {
      m.react("ℹ️");
      return m.reply(Func.texted("bold", `Usage: Reply to a text message with .copilot to use copilot AI.`));
    }
    const queryText = quoted.text;
    m.react("⏱️");
    try {
      const apiUrl = API("wudysoft", "/api/ai/copilot");
      ...
      if (response && response.data && response.data.result) {
        ...
        m.react("❌");
        return m.reply(Func.texted("bold", "No result received from copilot AI."));
      }
    } catch (error) {
      console.error(error);
      m.react("❌");

module.exports = {
  ...
  run: async (m, {
    ...
    if (!m.args || !m.args[0]) return m.reply(Func.example(m.prefix, m.command, "tiktok"));
    ...
    if (!pluginFile) return m.reply(Func.texted("bold", `Plugin ${pluginName}.js not found.`));
    const fileName = pluginFile.split("/").pop();
    if (setting.pluginDisable.includes(fileName)) {
      if (m.command === "plugdis") {
        return m.reply(Func.texted("bold", `Plugin ${fileName} is already disabled.`));
      }
    }
    if (m.command === "plugdis") {
      if (!setting.pluginDisable.includes(fileName)) {
        setting.pluginDisable.push(fileName);
        m.reply(Func.texted("bold", `Plugin ${fileName} successfully disabled.`));
      } else {
        return m.reply(Func.texted("bold", `Plugin ${fileName} is already disabled.`));
      }
      ...
      if (setting.pluginDisable.includes(fileName)) {
        ...
        m.reply(Func.texted("bold", `Plugin ${fileName} successfully enabled.`));
      } else {
        return m.reply(Func.texted("bold", `Plugin ${fileName} is not disabled.`));
      }
    }
  },
  owner: true

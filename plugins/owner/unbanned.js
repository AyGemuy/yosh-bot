module.exports = {
  ...
  run: async (m, {
    ...
    if (m.isGroup) {
      ...
      if (isOwner) who = m.mentions[0] ? m.mentions[0] : m.quoted ? m.quoted.sender : m.text ? m.text.replace(/[^0-9]/g, "") + "@s.whatsapp.net" : m.chat;
      who = m.mentions[0] ? m.mentions[0] : m.text ? m.text.replace(/[^0-9]/g, "") + "@s.whatsapp.net" : m.chat;
    } else {
      if (!isOwner) {
        m.reply(mess.owner);
        return false;

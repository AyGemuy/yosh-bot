module.exports = {
  ...
  run: async (m, {
    ...
    if (isNaN(m.args[1])) return m.reply(`Hanya format angka mewakili jumlah hari.`);
    m.reply(mess.wait);
    try {
      ...
      let e = await d.filter(v => v.endsWith(owner.number + "@s.whatsapp.net"));
      let jumlahHari = 864e5 * m.args[1];
      let now = new Date() * 1;

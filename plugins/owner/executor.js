...
handler.before = async function(m, _2) {
  ...
  if (m.body.startsWith("=>")) {
    try {
      ...
      let execFunc = new(async () => {}).constructor("print", "m", "handler", "require", "conn", "Func", "setting", "store", "quoted", "Array", "process", "plugins", "module", "exports", "argument", txt);
      _return = await execFunc.call(conn, (...args) => {
        if (--i < 1) return;
        console.log(...args);
        return m.reply(format(...args));
      }, m, handler, require, conn, Func, setting, store, quoted, CustomArray, process, plugins, f, f.exports, [conn, _2]);
    } catch (e) {

module.exports = {
  ...
  run: async (m, {
    ...
    if (!text && !m.isQuoted) return m.reply(Func.texted("bold", `Reply/tag user`));
    if (isNaN(number)) return m.reply(Func.texted("bold", `Nomor yang kamu masukkan tidak valid!`));
    if (number.length > 15) return m.reply(Func.texted("bold", `Nomor yang kamu masukkan tidak valid!`));
    try {
      if (m.text) {
      ...
      let groupMetadata = m.isGroup ? await store.groupMetadata[m.chat] : {};
